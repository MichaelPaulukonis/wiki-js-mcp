...
async def wikijs_create_page(
    title: str, content: str, space_id: str = "", parent_id: str = ""
) -> str:
    """
    ...
    try:
        ...
        if parent_id:
            ...
            """
            parent_response = await wikijs.graphql_request(
                parent_query, {"id": int(parent_id)}
            )
        ...
        if response_result.get("succeeded"):
            ...
            logger.info(
                f"Created page: {title} (ID: {result['pageId']}) at path: {path}"
            )
...
async def wikijs_update_page(
    page_id: int, title: str = None, content: str = None
) -> str:
    """
...
async def wikijs_search_pages(query: str, space_id: str = None) -> str:
    ...
    try:
        ...
        return json.dumps(
            {"results": results, "total": search_data.get("totalHits", len(results))}
        )
...
async def wikijs_create_space(name: str, description: str = None) -> str:
    ...
    try:
        ...
        if "error" not in result_data:
            ...
            logger.info(
                f"Created space (root page): {name} (ID: {space_result['spaceId']})"
            )
...
async def wikijs_link_file_to_page(
    file_path: str, page_id: int, relationship: str = "documents"
) -> str:
    """
    ...
    try:
        ...
        mapping = (
            db.query(FileMapping).filter(FileMapping.file_path == file_path).first()
        )
        if mapping:
...
async def wikijs_sync_file_docs(
    file_path: str, change_summary: str, snippet: str = None
) -> str:
    """
    ...
    try:
        ...
        if "error" in update_data:
            return json.dumps(
                {"error": f"Failed to update page: {update_data['error']}"}
            )
        ...
        if target_page_id:
            ...
            if "error" not in result_data:
                ...
                if "pageId" in result_data:
                    await wikijs_link_file_to_page(
                        file_path, result_data["pageId"], "documents"
                    )
...
async def wikijs_bulk_update_project_docs(
    ...
    try:
        db = get_db()
        results = {"updated_pages": [], "created_pages": [], "errors": []}
        ...
        for file_path in affected_files:
            try:
                ...
                mapping = (
                    ...
                    .filter(FileMapping.file_path == file_path)
                    .first()
                )
                ...
                if mapping:
                    ...
                    sync_response = await wikijs_sync_file_docs(
                        file_path, f"Bulk update: {summary}", context
                    )
                    sync_data = json.loads(sync_response)
                    if "error" not in sync_data:
                        results["updated_pages"].append(
                            {"file_path": file_path, "page_id": mapping.page_id}
                        )
                    else:
                        results["errors"].append(
                            {"file_path": file_path, "error": sync_data["error"]}
                        )
                    ...
                    if "error" not in overview_data and "pageId" in overview_data:
                        results["created_pages"].append(
                            {"file_path": file_path, "page_id": overview_data["pageId"]}
                        )
                    else:
                        results["errors"].append(
                            {
                                "file_path": file_path,
                                "error": overview_data.get(
...
async def wikijs_manage_collections(
    collection_name: str, description: str = None, space_ids: List[int] = None
) -> str:
    """
...
async def wikijs_repository_context() -> str:
    ...
    try:
        ...
        context = (
            db.query(RepositoryContext)
            .filter(RepositoryContext.root_path == repo_root)
            .first()
        )

        # Get file mappings for this repository
        mappings = (
            db.query(FileMapping).filter(FileMapping.repository_root == repo_root).all()
        )

        result = {
            "repository_root": repo_root,
            "space_name": (
                context.space_name if context else settings.DEFAULT_SPACE_NAME
            ),
...
async def wikijs_create_repo_structure(
    repo_name: str, description: str = None, sections: List[str] = None
) -> str:
    """
...
"""
        ...
        if "error" in root_data:
            return json.dumps(
                {"error": f"Failed to create root page: {root_data['error']}"}
            )
...
"""
            ...
            section_result = await wikijs_create_page(
                section, section_content, parent_id=str(root_page_id)
            )
            ...
            if "error" not in section_data:
                ...
                logger.warning(
                    f"Failed to create section '{section}': {section_data['error']}"
                )
        ...
        logger.info(
            f"Created repository structure for {repo_name} with {len(created_pages)} pages"
        )
        return json.dumps(result)
...
async def wikijs_create_nested_page(
    title: str, content: str, parent_path: str, create_parent_if_missing: bool = True
) -> str:
    """
    ...
    try:
        ...
        parent_response = await wikijs.graphql_request(
            parent_query, {"path": parent_path}
        )
        parent_data = (
            parent_response.get("data", {}).get("pages", {}).get("singleByPath")
        )
        ...
        if not parent_data and create_parent_if_missing:
            ...
            for i, part in enumerate(path_parts):
                ...
                check_response = await wikijs.graphql_request(
                    parent_query, {"path": current_path}
                )
                existing = (
                    check_response.get("data", {}).get("pages", {}).get("singleByPath")
                )
...
"""
                    ...
                    create_result = await wikijs_create_page(
                        part_title,
                        part_content,
            ...
            return json.dumps(
                {
                    "error": f"Parent path '{parent_path}' not found and create_parent_if_missing is False"
                }
            )
...
async def wikijs_create_documentation_hierarchy(
    project_name: str, file_mappings: List[Dict[str, str]], auto_organize: bool = True
) -> str:
    """
    ...
    try:
        ...
        section_names = (
            [name.title() for name, files in sections.items() if files]
            if auto_organize
            else ["Documentation"]
        )

        repo_result = await wikijs_create_repo_structure(
            project_name, f"Documentation for {project_name}", section_names
        )
        ...
        if auto_organize:
            ...
            for section_name, files in sections.items():
                ...
                for file_mapping in files:
                    ...
                    doc_path = file_mapping.get(
                        "doc_path", slugify(os.path.basename(file_path))
                    )

                    # Generate documentation content for the file
                    file_overview_result = await wikijs_generate_file_overview(
                        file_path, target_page_id=None
                    )
                    ...
                    if "error" not in overview_data:
                        ...
                        # Create mapping
                        mapping_result = await wikijs_link_file_to_page(
                            file_path, overview_data["pageId"], "documents"
                        )
            ...
            for file_mapping in file_mappings:
                file_path = file_mapping["file_path"]
                doc_path = file_mapping.get(
                    "doc_path", f"{project_name}/{slugify(os.path.basename(file_path))}"
                )
                ...
                if "error" not in nested_data:
                    ...
                    # Create mapping
                    mapping_result = await wikijs_link_file_to_page(
                        file_path, nested_data["pageId"], "documents"
                    )
                    mapping_data = json.loads(mapping_result)
...
async def wikijs_delete_page(
    page_id: int = None, page_path: str = None, remove_file_mapping: bool = True
) -> str:
    """
    ...
    try:
        ...
        if page_id:
            ...
            page_data = (
                get_response.get("data", {}).get("pages", {}).get("singleByPath")
            )
            if page_data:
        ...
        if response_result.get("succeeded"):
            ...
            if remove_file_mapping:
                db = get_db()
                mapping = (
                    db.query(FileMapping).filter(FileMapping.page_id == page_id).first()
                )
                if mapping:
                    db.delete(mapping)
                    db.commit()
    ...
    try:
        ...
        if page_paths:
            for page_path in page_paths:
                ...
                """
                get_response = await wikijs.graphql_request(
                    get_query, {"path": page_path}
                )
                page_data = (
                    get_response.get("data", {}).get("pages", {}).get("singleByPath")
                )
        ...
        for page in pages_to_delete:
            try:
                delete_result = await wikijs_delete_page(
                    page_id=page["id"], remove_file_mapping=remove_file_mappings
                )
        ...
        logger.info(
            f"Batch deletion completed: {len(deleted_pages)} deleted, {len(failed_deletions)} failed"
        )
        return json.dumps(result)
    ...
    try:
        ...
        result = {
            ...
            "hierarchy_summary": {
                ...
                "max_depth": (
                    max([p["path"].count("/") for p in pages_to_delete])
                    if pages_to_delete
                    else 0
                ),
            },
...
async def wikijs_cleanup_orphaned_mappings() -> str:
    ...
    try:
        ...
        if not mappings:
            return json.dumps({"message": "No file mappings found", "cleaned_count": 0})
...
async def wikijs_list_assets(folder_id: int = 0, kind: str = "ALL") -> str:
    ...
    try:
        ...
        response = await wikijs.graphql_request(
            query, {"folderId": folder_id, "kind": kind}
        )
...
@mcp.tool()
async def wikijs_create_asset_folder(
    name: str, slug: str, parent_folder_id: int = 0
) -> str:
    """
    ...
    try:
        ...
        response = await wikijs.graphql_request(
            mutation, {"parentFolderId": parent_folder_id, "slug": slug, "name": name}
        )
        ...
        if response_result.get("succeeded"):
            logger.info(
                f"Created asset folder: {name} (slug: {slug}, parent: {parent_folder_id})"
            )
            return json.dumps(
                {
                    "created": True,
